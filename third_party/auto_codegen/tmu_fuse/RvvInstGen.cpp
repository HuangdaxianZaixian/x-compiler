...
bool RvvInst::isExclusiveOneBundleInst(const RvvInst& inst) {
    return inst.assembly.find("loop_cfgx") == 0 || 
           inst.assembly.find("loop_end") == 0 ||
           inst.assembly.find("vsetvli") == 0;
}

bool RvvInst::isNopInst(const RvvInst& inst) {
    return inst.assembly.find("nop") == 0;
}
...
bool RvvInst::isUsingGeneralVectorRegister() const {
    for (const auto& input : inputs) {
        if (input.type == RvvOperandType::VREG && (input.value.v_reg.id >= 0 && input.value.v_reg.id <= 15)) {
            return true;
        }
    }
    if (output.type == RvvOperandType::VREG && (output.value.v_reg.id >= 0 && output.value.v_reg.id <= 15)) {
        return true;
    }
    return false;
}

bool RvvInst::isUsingVirtualVectorRegisterV24OrV25() const {
    if (output.type == RvvOperandType::VREG && (output.value.v_reg.id == 24 || output.value.v_reg.id == 25)) {
        return true;
    }
    return false;
}

bool RvvInst::isUsingVirtualVectorRegisterV23() const {
    for (const auto& input : inputs) {
        if (input.type == RvvOperandType::VREG && (input.value.v_reg.id == 23)) {
            return true;
        }
    }
    if (output.type == RvvOperandType::VREG && (output.value.v_reg.id == 23)) {
        return true;
    }
    return false;
}
